...
<xsl:template match="params">
	...
	<style>
		.cl-slidename-bg-var { width: <xsl:value-of select="number($width)-2*number(border_width)"/>px; height: <xsl:value-of select="number($height)-2*number(border_width)"/>px; <xsl:if test="display_bg='yes'"><xsl:if test="radius!='0'">border-radius: <xsl:value-of select="radius"/>px;</xsl:if> border-style: solid; border-width: <xsl:value-of select="border_width"/>px; border-color: <xsl:value-of select="$border-color-fixed"/>; background-color: <xsl:value-of select="$_fill"/>; <xsl:if test="fill_type='auto' or fill_type='custom'">background-image: linear-gradient(<xsl:value-of select="$gradient-angle"/>deg, <xsl:value-of select="$bg-color-fixed"/>, <xsl:value-of select="$color-gradient"/>);</xsl:if><xsl:if test="$_shadow_string!=''">box-shadow: <xsl:value-of select="$_shadow_string"/>;</xsl:if></xsl:if> }
		.cl-slidename-table-var { width: <xsl:value-of select="number($width)-2*number(border_width)"/>px; height: <xsl:value-of select="number($height)-2*number(border_width)"/>px;  }
		.cl-slidename-txt-box-var { <xsl:if test="text_shadow_strength!='none'">text-shadow: <xsl:value-of select="$_text_shadow"/>;</xsl:if> padding: <xsl:value-of select="text_padding_v"/>px <xsl:value-of select="text_padding_h"/>px; text-align: <xsl:value-of select="text_align"/>; color: <xsl:value-of select="$font-color-fixed"/>; font-size: <xsl:value-of select="font_size"/>px; <xsl:value-of select="$_css_font_family"/><xsl:value-of select="$_css_font_weight"/><xsl:value-of select="$_css_font_style"/> }
	</style>
	<div>
