...
<xsl:template match="params">
	...
	<xsl:variable name="_css_font">
		font-family: <xsl:value-of select="$_btn_font"/>; font-size: <xsl:value-of select="font_size"/>px; color: <xsl:value-of select="$font-color-fixed"/>; <xsl:if test="$_text_shadow!=''">text-shadow: <xsl:value-of select="$_text_shadow"/>;</xsl:if><xsl:value-of select="$_css_font_style"/>
	</xsl:variable>
	...
	<style>
		...
		<xsl:if test="btn_type='html'">
			.cl-btn-html { <xsl:value-of select="$_css_border"/> <xsl:value-of select="$_css_shadow"/> }
			.cl-idle { border-color: <xsl:value-of select="$border-color-fixed"/>; <xsl:value-of select="$_css_bg_color"/> <xsl:value-of select="$_css_gradient_style"/> }
			...
			.cl-btn-table-content { width: <xsl:value-of select="$_btn_inner_w"/>px; height: <xsl:value-of select="$_btn_inner_h"/>px; text-align: <xsl:value-of select="text_position"/>; padding: 0 <xsl:value-of select="text_margin_right"/>px 0 <xsl:value-of select="text_margin_left"/>px;  <xsl:value-of select="$_css_font"/> }
		</xsl:if>
	</style>
	<div class="clo-nav_0000_btn_navi">
		<div>
