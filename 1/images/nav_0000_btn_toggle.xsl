...
<xsl:template match="params">
	...
	<xsl:variable name="gradient-color-fixed">
 	...
 	<xsl:variable name="bg-color-off-fixed">
		<xsl:call-template name="fix-color">
			<xsl:with-param name="color" select="bg_color_off"/>
		</xsl:call-template>
	</xsl:variable>
	<xsl:variable name="gradient-color-off-fixed">
		<xsl:call-template name="fix-color">
			<xsl:with-param name="color" select="gradient_color_off"/>
		</xsl:call-template>
	</xsl:variable>
	...
	<xsl:variable name="_btn_text">
		<xsl:choose>
			<xsl:when test="role='sound'"><xsl:value-of select="text_on_sound"/></xsl:when>
			<xsl:when test="role='fitscreen'"><xsl:value-of select="text_on_fitscreen"/></xsl:when>
			<xsl:when test="role='method'"><xsl:value-of select="text_on_method"/></xsl:when>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="_btn_text_off">
		<xsl:choose>
			<xsl:when test="role='sound'"><xsl:value-of select="text_off_sound"/></xsl:when>
			<xsl:when test="role='fitscreen'"><xsl:value-of select="text_off_fitscreen"/></xsl:when>
			<xsl:when test="role='method'"><xsl:value-of select="text_off_method"/></xsl:when>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="_alt_text">
		<xsl:choose>
			<xsl:when test="role='sound'"><xsl:value-of select="alt_on_sound"/></xsl:when>
			<xsl:when test="role='fitscreen'"><xsl:value-of select="alt_on_fitscreen"/></xsl:when>
			<xsl:when test="role='method'"><xsl:value-of select="alt_on_method"/></xsl:when>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="_alt_text_off">
		<xsl:choose>
			<xsl:when test="role='sound'"><xsl:value-of select="alt_off_sound"/></xsl:when>
			<xsl:when test="role='fitscreen'"><xsl:value-of select="alt_off_fitscreen"/></xsl:when>
			<xsl:when test="role='method'"><xsl:value-of select="alt_off_method"/></xsl:when>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="_btn_gradient_color_off">
		<xsl:choose>
			<xsl:when test="bg_type='auto'">
				<xsl:call-template name="autogradient">
					<xsl:with-param name="color.base" select="$bg-color-off-fixed"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="bg_type='custom'"><xsl:value-of select="$gradient-color-off-fixed"/></xsl:when>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="_css_border">
		<xsl:choose>
			<xsl:when test="number(border_width)=0 or btn_type='image' or border_style='none'"></xsl:when>
			...
			<xsl:when test="btn_type='html'">
				<div class="rule">
					...
					<xsl:if test="number(border_width)!=0">
						<span class="rule-static"><xsl:value-of select="$_css_border"/></span>
					</xsl:if>
				...
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-btn-on .cl-idle</xsl:attribute>
				...
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-btn-on .cl-over</xsl:attribute>
				...
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-btn-off .cl-idle</xsl:attribute>
					<xsl:if test="bg_type!='transparent'">
						<span class="rule-static">background-color: <xsl:value-of select="$bg-color-off-fixed"/>;</span>
					</xsl:if>
					<xsl:if test="bg_type='auto' or bg_type='custom'">
						<span class="rule-dynamic">
							<xsl:attribute name="data-type">linear-gradient</xsl:attribute>
							<xsl:attribute name="data-angle">180</xsl:attribute>
							<xsl:attribute name="data-colors"><xsl:value-of select="$bg-color-off-fixed"/>|0;<xsl:value-of select="$_btn_gradient_color_off"/>|100</xsl:attribute>
						</span>
					</xsl:if>
				</div>
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-btn-off .cl-over</xsl:attribute>
					<span class="rule-static">background-color: <xsl:value-of select="$_btn_gradient_color_off"/>;</span>
					<xsl:if test="bg_type='auto' or bg_type='custom'">
						<span class="rule-dynamic">
							<xsl:attribute name="data-type">linear-gradient</xsl:attribute>
							<xsl:attribute name="data-angle">180</xsl:attribute>
							<xsl:attribute name="data-colors"><xsl:value-of select="$_btn_gradient_color_off"/>|0;<xsl:value-of select="$bg-color-off-fixed"/>|100</xsl:attribute>
						</span>
					</xsl:if>
				</div>
				<div class="rule">
				...
				<xsl:if test="content='image' or content='textimage'">
					<div class="rule">
						<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-btn-on .cl-idle .cl-btn-bg</xsl:attribute>
					...
					<div class="rule">
						<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-btn-on .cl-over .cl-btn-bg</xsl:attribute>
					...
					<div class="rule">
						<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-btn-off .cl-idle .cl-btn-bg</xsl:attribute>
					...
					<div class="rule">
						<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-btn-off .cl-over .cl-btn-bg</xsl:attribute>
						<span class="rule-static">background-image: url(<xsl:value-of select="bg_img_4"/>); background-position: <xsl:value-of select="bg_position"/> center; background-repeat: no-repeat;</span>
					</div>
				</xsl:if>
				<div class="rule">
		...
		<xsl:choose>
			<xsl:when test="btn_type='image'">
				<div>
					...
					<xsl:attribute name="class">cl-btn cl-btn-img cl-btn-on</xsl:attribute>
					<xsl:attribute name="title"><xsl:value-of select="$_alt_text"/></xsl:attribute>
					<xsl:attribute name="data-title-on"><xsl:value-of select="$_alt_text"/></xsl:attribute>
					<xsl:attribute name="data-title-off"><xsl:value-of select="$_alt_text_off"/></xsl:attribute>
					<div>
						<xsl:attribute name="class">cl-idle</xsl:attribute>
						<img>
						...
						<img>
							<xsl:attribute name="class">cl-img-off-idle</xsl:attribute>
							<xsl:attribute name="src"><xsl:value-of select="n3_img"/></xsl:attribute>
						</img>
						<img>
							<xsl:attribute name="class">cl-img-off-over</xsl:attribute>
			...
			<xsl:when test="btn_type='html'">
				<div>
					<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_BTN</xsl:attribute>
					<xsl:attribute name="class">cl-btn cl-btn-html cl-btn-on</xsl:attribute>
					<xsl:attribute name="title"><xsl:value-of select="$_alt_text"/></xsl:attribute>
					<xsl:attribute name="data-title-on"><xsl:value-of select="$_alt_text"/></xsl:attribute>
					<xsl:attribute name="data-title-off"><xsl:value-of select="$_alt_text_off"/></xsl:attribute>
					<div>
								...
								<tr>
									<td>
										...
										<xsl:choose>
											...
											<xsl:otherwise>
												<span>
													<xsl:attribute name="class">cl-text-on</xsl:attribute>
													<xsl:value-of select="$_btn_text"/>
												</span>
												<span>
													<xsl:attribute name="class">cl-text-off</xsl:attribute>
													<xsl:value-of select="$_btn_text_off"/>
												</span>
											</xsl:otherwise>
										</xsl:choose>
									</td>
