...
<xsl:template match="params">
	...
	<xsl:variable name="_css_gradient_style">
		<xsl:if test="bg_type='auto' or bg_type='custom'">background-image: -webkit-linear-gradient(90deg, <xsl:value-of select="$bg-color-fixed"/> 0%, <xsl:value-of select="$_btn_gradient_color"/> 100%); background-image: linear-gradient(0deg, <xsl:value-of select="$bg-color-fixed"/> 0%, <xsl:value-of select="$_btn_gradient_color"/> 100%);</xsl:if>
	</xsl:variable>
	<xsl:variable name="_css_shadow">
		<xsl:if test="$_shadow_string!=''">-webkit-box-shadow: <xsl:value-of select="$_shadow_string"/>; box-shadow: <xsl:value-of select="$_shadow_string"/>;</xsl:if>
	</xsl:variable>
	...
	<xsl:variable name="_css_bg_img">
		<xsl:if test="content='image' or content='textimage'">background-image: url('file:///<xsl:value-of select="translate(substring-before($moduleImagesFolder,'images\'),'\','/')"/><xsl:value-of select="bg_img_1"/>'); background-position: <xsl:value-of select="bg_position"/> center; background-repeat: no-repeat;</xsl:if>
	</xsl:variable>
	<style>
		.cl-btn { position: relative; left: 0; top: 0; box-sizing: border-box; width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px; }
		.cl-btn-html { border-style: solid; <xsl:value-of select="$_css_border"/> <xsl:value-of select="$_css_shadow"/> }
		.idle { border-color: <xsl:value-of select="$border-color-fixed"/>; <xsl:value-of select="$_css_bg_color"/> <xsl:value-of select="$_css_gradient_style"/> }
		.cl-btn-bg { position: relative; padding: 0; margin: 0; width: <xsl:value-of select="$_btn_inner_w"/>px; height: <xsl:value-of select="$_btn_inner_h"/>px; <xsl:value-of select="$_css_bg_img"/> }
		.cl-btn-table { border-spacing: 0; width: <xsl:value-of select="$_btn_inner_w"/>px; height: <xsl:value-of select="$_btn_inner_h"/>px; }
		.cl-btn-table td { text-align: center; vertical-align: middle; padding: 0; text-shadow: <xsl:value-of select="$_text_shadow"/>; }
		.cl-btn-table .cl-btn-table-content { width: <xsl:value-of select="$_btn_inner_w"/>px; height: <xsl:value-of select="$_btn_inner_h"/>px; <xsl:value-of select="$_css_font"/> }
	</style>
		...
		<xsl:choose>
			<xsl:when test="btn_type='image'">
				<div>
					<xsl:attribute name="style">position: absolute; width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px;</xsl:attribute>
					<img>
						<xsl:attribute name="style">position: absolute; left: 0; top: 0;</xsl:attribute>
				...
				<div>
					...
					<img>
						<xsl:attribute name="class">img-idle</xsl:attribute>
						<xsl:attribute name="src"><xsl:value-of select="substring-before($moduleImagesFolder,'images\')"/><xsl:value-of select="translate(n1_img,'/','\')"/></xsl:attribute>
					</img>
					<img>
						<xsl:attribute name="class">img-over</xsl:attribute>
						<xsl:attribute name="src"><xsl:value-of select="substring-before($moduleImagesFolder,'images\')"/><xsl:value-of select="translate(n2_img,'/','\')"/></xsl:attribute>
					</img>
				</div>
			</xsl:when>
			<xsl:when test="btn_type='html'">
				<div>
					<xsl:attribute name="class">cl-btn cl-btn-html idle</xsl:attribute>
					<div>
