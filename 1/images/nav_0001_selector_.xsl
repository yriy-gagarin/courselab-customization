...
<xsl:template match="params">
	...
	<style>
		.cl-selector { width: <xsl:value-of select="$width"/>px; height: <xsl:value-of select="$height"/>px; padding: <xsl:value-of select="padding"/>px; text-align: <xsl:value-of select="text_align" />; border-width: <xsl:value-of select="border_width" />px; border-color: <xsl:value-of select="$border-color-fixed" />; border-style: <xsl:value-of select="border_style" />; background-color: <xsl:value-of select="$bg-color-fixed" />; <xsl:value-of select="$main.sFontCSS" /><xsl:value-of select="$main.sRadiusCSS" /><xsl:value-of select="$main.sShadowCSS" /> }
		.cl-selector option { background-color: <xsl:value-of select="$bg-color-fixed" />; <xsl:value-of select="$main.sFontCSS" /> }
	</style>
	<div>
		<xsl:attribute name="class">clo-nav_0001_selector</xsl:attribute>
		<div>
			<xsl:attribute name="class">cl-container</xsl:attribute>
			<select>
				<xsl:attribute name="class">cl-selector</xsl:attribute>
				<option value="0">
					<xsl:value-of select="hidden.label.default"/>
				</option>
			</select>
		</div>
	</div>
