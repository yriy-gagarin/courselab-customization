...
<xsl:template match="params">
	...
	<xsl:variable name="font-color-over-fixed">
		<xsl:call-template name="fix-color">
			<xsl:with-param name="color" select="font_color_over_color"/>
		</xsl:call-template>
	</xsl:variable>
	...
	<xsl:variable name="_css_gradient_style">
		<xsl:if test="bg_type='auto' or bg_type='custom'">background-image: linear-gradient(0deg, <xsl:value-of select="$bg-color-fixed"/> 0%, <xsl:value-of select="$_btn_gradient_color"/> 100%);</xsl:if>
	</xsl:variable>
	<xsl:variable name="_css_shadow">
		<xsl:if test="$_shadow_string!=''">box-shadow: <xsl:value-of select="$_shadow_string"/>;</xsl:if>
	</xsl:variable>
	<xsl:variable name="_css_border_radius">
		<xsl:if test="number(border_radius)!='0'">border-radius: <xsl:value-of select="border_radius"/>px;</xsl:if>
	</xsl:variable>

	<xsl:variable name="_css_border">
		<xsl:choose>
			<xsl:when test="btn_type='image'"></xsl:when>
	...
	<style>
		...
		.cl-btn-html { <xsl:value-of select="$_css_border"/> <xsl:value-of select="$_css_shadow"/> }
		.cl-idle { <xsl:value-of select="$_css_bg_color"/> <xsl:value-of select="$_css_gradient_style"/> }
		.cl-btn-bg { width: <xsl:value-of select="$_btn_inner_w"/>px; height: <xsl:value-of select="$_btn_inner_h"/>px; <xsl:value-of select="$_css_bg_img"/> }
		.cl-btn-table { width: <xsl:value-of select="$_btn_inner_w"/>px; height: <xsl:value-of select="$_btn_inner_h"/>px; }
		.cl-btn-table .cl-btn-table-content { width: <xsl:value-of select="$_btn_inner_w"/>px; height: <xsl:value-of select="$_btn_inner_h"/>px; text-align: <xsl:value-of select="text_position"/>; padding: 0 <xsl:value-of select="text_margin_right"/>px 0 <xsl:value-of select="text_margin_left"/>px; <xsl:value-of select="$_css_font"/> }
	</style>
	<div class="clo-nav_0000_btn_toggle">
		<div>
