		...
		<div>
			...
			<select>
				...
				<xsl:for-each select="valuelist/item">
					<option>
						<xsl:attribute name="style"><xsl:value-of select="$css-cl-fld-option"/></xsl:attribute>
						<xsl:if test="input_checked='yes'"><xsl:attribute name="selected">true</xsl:attribute></xsl:if>
						<xsl:attribute name="value"><xsl:choose><xsl:when test="input_value!=''"><xsl:value-of select="input_value"/></xsl:when><xsl:otherwise><xsl:value-of select="position()"/></xsl:otherwise></xsl:choose></xsl:attribute>
