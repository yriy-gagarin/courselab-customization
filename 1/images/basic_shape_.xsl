...
<xsl:template match="params">
	<xsl:variable name="bDesign">yes</xsl:variable>
	...
	<xsl:variable name="text-box-width"><xsl:value-of select="number($main.text.w) - number(text_margin_left) - number(text_margin_right)"/></xsl:variable>
	<xsl:variable name="text-box-min-width">
		<xsl:choose>
			<xsl:when test="number($text-box-width) &gt; 0"><xsl:value-of select="$text-box-width"/></xsl:when>
			<xsl:otherwise>1</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="text-box-height"><xsl:value-of select="number($main.text.h) - number(text_margin_top) - number(text_margin_bottom)"/></xsl:variable>
	...
	<xsl:variable name="css-cl-btn-text">padding: <xsl:value-of select="text_margin_top"/>px <xsl:value-of select="text_margin_right"/>px <xsl:value-of select="text_margin_bottom"/>px <xsl:value-of select="text_margin_left"/>px; vertical-align: <xsl:value-of select="$txt.valign"/>; text-align: <xsl:value-of select="$txt.align"/>; width: <xsl:value-of select="$text-box-min-width"/>px; height: <xsl:value-of select="$text-box-min-height"/>px; <xsl:value-of select="$_css_font"/> <xsl:value-of select="$_css_onimg_bg"/></xsl:variable>

	<div>
		<xsl:attribute name="class">clo-basic_shape</xsl:attribute>
		<div>
	...
	<xsl:if test="number($text-box-min-width)=1 or number($text-box-min-height)=1">
		<div>
