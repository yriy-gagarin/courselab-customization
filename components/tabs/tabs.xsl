...
function sum(aV) { var aVal = aV; if(typeof aVal != 'object') { aVal = eval(aV) } var i = 0; var iSum = 0; while(aVal[i]!=null) { iSum += aVal[i]; i++; } return iSum; }
function arrayPush(sArrayString, sNewValue, sDivider) { var aArray = sArrayString.split(sDivider); aArray.push(sNewValue); return aArray.join(sDivider); }
...
function arraySum(sArrayString, iIndex1, iIndex2, sDivider) { var aArray = sArrayString.split(sDivider); var iIdx1 = parseInt(iIndex1, 10); var iIdx2 = parseInt(iIndex2, 10); if(isNaN(iIdx1) || isNaN(iIdx2)) return 0; if(iIdx1==iIdx2) return 0; if(iIdx2==0) iIdx2 = aArray.length + 1; iIdx1 -= 1; iIdx2 -= 1; if(iIdx1!=Math.abs(iIdx1) || iIdx2!=Math.abs(iIdx2)) return 0; if((iIdx2-iIdx1)!=Math.abs(iIdx2-iIdx1)) return 0; if(aArray[iIdx1]==null || aArray[iIdx2]==null) return 0; var nSum = 0; var nVal = 0; for(var i=iIdx1; i!=iIdx2; i++) { nVal = parseFloat(aArray[i]); if(isNaN(nVal)) continue; nSum += nVal; } return nSum; }
function radialToXY(sCoord, nCenter, nRadius, nAngle) { return ((sCoord=="x") ? (nCenter + nRadius*Math.cos( (nAngle-90)*Math.PI/180 )) : (nCenter + nRadius*Math.sin( (nAngle-90)*Math.PI/180 ))); }
...
<xsl:template match="params">
	...
	<div class="cl-audio-files-list" style="display: none">
		<xsl:for-each select="elements/item">
			<xsl:if test="narration!=''"><span><xsl:value-of select="narration"/></span></xsl:if>
		</xsl:for-each>
	...
	<xsl:variable name="main.nShadowOffset">6</xsl:variable>
	<xsl:variable name="main.nDescWrapperMargin">
		<xsl:choose>
			<xsl:when test="number(desc_border_width) &lt; 6">3</xsl:when>
			<xsl:otherwise><xsl:value-of select="0.5*number(desc_border_width)"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.nDescBGRadius" select="number(desc_radius) + number($main.nDescWrapperMargin)"/>
	...
	<xsl:variable name="main.nDescPartWidth">
		<xsl:choose>
			<xsl:when test="tab_type='left' or tab_type='right'"><xsl:value-of select="number($main.nInnerWidth) - number($main.nTabPartWidth)"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="$main.nInnerWidth"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<xsl:variable name="main.nDescPartHeight">
		<xsl:choose>
			<xsl:when test="tab_type='top' or tab_type='bottom'"><xsl:value-of select="number($main.nTabBlockHeight) - number($main.nTabPartHeight)"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="$main.nTabBlockHeight"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="tab_type='left' or tab_type='right'">
						<xsl:choose>
							<xsl:when test="count(elements/item)=1 or count(elements/item)=0"><xsl:value-of select="number($main.nTabPartHeight) - 2*number(desc_border_width)"/></xsl:when>
				...
				<xsl:choose>
					<xsl:when test="tab_type='top' or tab_type='bottom'">
						<xsl:choose>
							...
							<xsl:otherwise><xsl:value-of select="((number($main.nTabPartWidth) - 2*number(desc_border_width)) div count(elements/item)) - 1"/></xsl:otherwise>
						</xsl:choose>
					</xsl:when>
	...
	<xsl:variable name="main.nTabCellWidth">
		<xsl:choose>
			<xsl:when test="(number($main.nTabWidth) - number(font_size)) &gt; 2*number(tab_padding_lr)"><xsl:value-of select="number($main.nTabWidth) - 2*number(tab_padding_lr) - 2*number(tab_border_width)"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="number($main.nTabWidth) - 2*number(tab_border_width)"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.nTabCellHeight">
		<xsl:choose>
			<xsl:when test="(number($main.nTabHeight) - number(font_size)) &gt; 2*number(tab_padding_tb)"><xsl:value-of select="number($main.nTabHeight) - 2*number(tab_padding_tb) - 2*number(tab_border_width)"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="number($main.nTabHeight) - 2*number(tab_border_width)"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.nTabCellPaddingLR">
		<xsl:choose>
			<xsl:when test="(number($main.nTabWidth) - number(font_size)) &gt; 2*number(tab_padding_lr)"><xsl:value-of select="tab_padding_lr"/></xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.nTabCellPaddingTB">
		<xsl:choose>
			<xsl:when test="(number($main.nTabHeight) - number(font_size)) &gt; 2*number(tab_padding_tb)"><xsl:value-of select="tab_padding_tb"/></xsl:when>
			<xsl:otherwise>0</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<div class="style-custom" style="display: none">
		...
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-bg</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="$main.nBGWidth"/>px; height: <xsl:value-of select="$main.nBGHeight"/>px; <xsl:if test="display_bg='yes'">padding: <xsl:value-of select="bg_padding"/>px; background-color: <xsl:value-of select="$main.sBGColor1"/>; border-width: <xsl:value-of select="bg_border_width"/>px; border-color: <xsl:value-of select="$main.sBGBorderColor"/>;</xsl:if></span>
			<xsl:if test="display_bg='yes'">
				...
				<xsl:if test="number(bg_radius)!=0">
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">border-radius</xsl:attribute>
						<xsl:attribute name="data-value"><xsl:value-of select="bg_radius"/>px</xsl:attribute>
					</span>
				</xsl:if>
				<xsl:if test="$main.sBGColor1!=$main.sBGColor2">
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">linear-gradient</xsl:attribute>
						<xsl:attribute name="data-angle"><xsl:value-of select="bg_grad_angle"/></xsl:attribute>
						<xsl:attribute name="data-colors"><xsl:value-of select="$main.sBGColor1"/>|0;<xsl:value-of select="$main.sBGColor2"/>|100</xsl:attribute>
					</span>
				</xsl:if>
			</xsl:if>
		</div>
		<xsl:if test="display_ribbon='yes'">
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-header-part</xsl:attribute>
				<span class="rule-static">border-color: <xsl:value-of select="$main.sRibbonBorderColor"/>; padding: 0 0 <xsl:value-of select="$main.nHeaderPadding"/>px 0; width: <xsl:value-of select="$main.nHeaderWidth"/>px; height: <xsl:value-of select="$main.nHeaderHeight"/>px;</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-header-table</xsl:attribute>
				<span class="rule-static">width: <xsl:value-of select="$main.nHeaderWidth"/>px; height: <xsl:value-of select="$main.nHeaderTableHeight"/>px;</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-header-cell-name</xsl:attribute>
				<span class="rule-static">width: <xsl:value-of select="number($main.nHeaderTableHeight) - 2*number($main.nHeaderTableHeight) - 4*number($main.nHeaderPadding)"/>px; height: <xsl:value-of select="$main.nHeaderTableHeight"/>px; padding: 0 <xsl:value-of select="2*number($main.nHeaderPadding)"/>px 0 0;</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-header-cell-btn</xsl:attribute>
				<span class="rule-static">width: <xsl:value-of select="$main.nHeaderTableHeight"/>px; height: <xsl:value-of select="$main.nHeaderTableHeight"/>px; padding: 0 0 0 <xsl:value-of select="$main.nHeaderPadding"/>px;</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-header-btn</xsl:attribute>
				<span class="rule-static">background-color: <xsl:value-of select="$main.sBtnColor1"/>; border-width: <xsl:value-of select="$main.nBtnBorderWidth"/>px; border-color: <xsl:value-of select="$main.sBtnColorBorder"/>; width: <xsl:value-of select="$main.nBtnSize"/>px; height: <xsl:value-of select="$main.nBtnSize"/>px;</span>
				<xsl:if test="number($main.nBtnRadius)!=0">
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">border-radius</xsl:attribute>
						<xsl:attribute name="data-value"><xsl:value-of select="$main.nBtnRadius"/></xsl:attribute>
					</span>
				</xsl:if>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-header-btn-idle</xsl:attribute>
				<xsl:if test="shadow_strength!='none'">
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">box-shadow</xsl:attribute>
						<xsl:attribute name="data-value"><xsl:value-of select="$main.sShadowString"/></xsl:attribute>
				...
				<span class="rule-dynamic">
					...
					<xsl:attribute name="data-colors"><xsl:value-of select="$main.sBtnColor1"/>|10;<xsl:value-of select="$main.sBtnColor3"/>|50;<xsl:value-of select="$main.sBtnColor4"/>|50;<xsl:value-of select="$main.sBtnColor2"/>|90</xsl:attribute>
				</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-header-btn-over</xsl:attribute>
				<xsl:if test="shadow_strength!='none'">
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">box-shadow</xsl:attribute>
						<xsl:attribute name="data-value"><xsl:value-of select="$main.sShadowString"/></xsl:attribute>
			...
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-arrow-left</xsl:attribute>
				<span class="rule-static">margin: -<xsl:value-of select="$main.nArrowBorderWidth"/>px 0 0 -<xsl:value-of select="round(0.66*number($main.nArrowBorderWidth))"/>px; border-width: <xsl:value-of select="$main.nArrowBorderWidth"/>px <xsl:value-of select="$main.nArrowBorderWidth"/>px <xsl:value-of select="$main.nArrowBorderWidth"/>px 0;</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-arrow-right</xsl:attribute>
				<span class="rule-static">margin: -<xsl:value-of select="$main.nArrowBorderWidth"/>px 0 0 -<xsl:value-of select="round(0.33*number($main.nArrowBorderWidth))"/>px; border-width: <xsl:value-of select="$main.nArrowBorderWidth"/>px 0 <xsl:value-of select="$main.nArrowBorderWidth"/>px <xsl:value-of select="$main.nArrowBorderWidth"/>px;</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-header-title</xsl:attribute>
				<span class="rule-static">width: <xsl:value-of select="number($main.nHeaderTableHeight) - 2*number($main.nHeaderTableHeight) - 4*number($main.nHeaderPadding)"/>px; color: <xsl:value-of select="$main.sRibbonBorderColor"/>; <xsl:value-of select="$main.sRibbonFontCSS"/> </span>
			</div>
		</xsl:if>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-block</xsl:attribute>
			<span class="rule-static">margin: <xsl:value-of select="$main.nHeaderPadding"/>px 0 0 0; width: <xsl:value-of select="$main.nInnerWidth"/>px; height: <xsl:value-of select="$main.nTabBlockHeight"/>px;</span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-desc-bg</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="number($main.nDescPartWidth) - 2*number(desc_border_width)"/>px; height: <xsl:value-of select="number($main.nDescPartHeight) - 2*number(desc_border_width)"/>px; border-width: <xsl:value-of select="desc_border_width"/>px;</span>
			<xsl:if test="shadow_strength!='none'">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">box-shadow</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:value-of select="$main.sShadowString"/></xsl:attribute>
		...
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-desc-bg-regular</xsl:attribute>
			<xsl:if test="(number($main.nDescBGRadius)!=0) and (count(elements/item) &gt; 1)">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">border-radius</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:value-of select="$main.nDescBGRadius"/>px</xsl:attribute>
				</span>
			</xsl:if>
		</div>
		<xsl:if test="number($main.nDescBGRadius)!=0">
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-desc-bg-single</xsl:attribute>
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">border-radius</xsl:attribute>
					<xsl:attribute name="data-important">1</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:choose><xsl:when test="tab_type='left'">0 <xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px 0</xsl:when><xsl:when test="tab_type='right'"><xsl:value-of select="$main.nDescBGRadius"/>px 0 0 <xsl:value-of select="$main.nDescBGRadius"/>px</xsl:when><xsl:when test="tab_type='top'">0 0 <xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px</xsl:when><xsl:when test="tab_type='bottom'"><xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px 0 0</xsl:when></xsl:choose></xsl:attribute>
				</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-desc-bg-first</xsl:attribute>
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">border-radius</xsl:attribute>
						<xsl:attribute name="data-important">1</xsl:attribute>
						<xsl:attribute name="data-value"><xsl:choose><xsl:when test="tab_type='left'">0 <xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px</xsl:when><xsl:when test="tab_type='right'"><xsl:value-of select="$main.nDescBGRadius"/>px 0 <xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px</xsl:when><xsl:when test="tab_type='top'">0 <xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px</xsl:when><xsl:when test="tab_type='bottom'"><xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px <xsl:value-of select="$main.nDescBGRadius"/>px 0</xsl:when></xsl:choose></xsl:attribute>
					</span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-desc-bg-last</xsl:attribute>
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">border-radius</xsl:attribute>
						<xsl:attribute name="data-important">1</xsl:attribute>
		...
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-desc-wrapper</xsl:attribute>
			<span class="rule-static">margin: <xsl:value-of select="$main.nDescWrapperMargin"/>px; padding: <xsl:value-of select="desc_padding"/>px; width: <xsl:value-of select="number($main.nDescWrapperWidth) - 2*number(desc_border_width) - 2*number(desc_padding)"/>px; height: <xsl:value-of select="number($main.nDescWrapperHeight) - 2*number(desc_border_width) - 2*number(desc_padding)"/>px; background-color: <xsl:value-of select="$main.sDescColor1"/>; border-width: <xsl:value-of select="desc_border_width"/>px; border-color: <xsl:value-of select="$main.sDescBorderColor"/>;</span>
			<xsl:if test="number(desc_radius)!=0">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">border-radius</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:value-of select="desc_radius"/>px</xsl:attribute>
				</span>
			</xsl:if>
			<xsl:if test="$main.sDescColor1!=$main.sDescColor2">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">linear-gradient</xsl:attribute>
					<xsl:attribute name="data-angle"><xsl:value-of select="desc_bg_grad_angle"/></xsl:attribute>
					<xsl:attribute name="data-colors"><xsl:value-of select="$main.sDescColor1"/>|0;<xsl:value-of select="$main.sDescColor2"/>|100</xsl:attribute>
				</span>
			</xsl:if>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-desc</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="number($main.nDescWrapperWidth) - 2*number(desc_border_width) - 2*number(desc_padding)"/>px; height: <xsl:value-of select="number($main.nDescWrapperHeight) - 2*number(desc_border_width) - 2*number(desc_padding)"/>px;</span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-part</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="$main.nTabPartWidth"/>px; height: <xsl:value-of select="$main.nTabPartHeight"/>px;</span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-bg</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="$main.nTabBGWidth"/>px; height: <xsl:value-of select="$main.nTabBGHeight"/>px;</span>
			<xsl:if test="shadow_strength!='none'">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">box-shadow</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:value-of select="$main.sShadowString"/></xsl:attribute>
		...
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-bg-left</xsl:attribute>
			<span class="rule-static">clip: rect(-<xsl:value-of select="$main.nShadowOffset"/>px, <xsl:value-of select="number($main.nTabBGWidth) + number(desc_border_width)"/>px, <xsl:value-of select="number($main.nTabBGHeight) + 2*number(desc_border_width) + number($main.nShadowOffset)"/>px, -<xsl:value-of select="$main.nShadowOffset"/>px); border-width: <xsl:value-of select="desc_border_width"/>px 0 <xsl:value-of select="desc_border_width"/>px <xsl:value-of select="desc_border_width"/>px;</span>
			<xsl:if test="number($main.nTabBGRadius)!=0">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">border-radius</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:value-of select="$main.nTabBGRadius"/>px 0 0 <xsl:value-of select="$main.nTabBGRadius"/>px</xsl:attribute>
				</span>
			</xsl:if>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-bg-right</xsl:attribute>
			<span class="rule-static">clip: rect(-<xsl:value-of select="$main.nShadowOffset"/>px, <xsl:value-of select="number($main.nTabBGWidth) + number(desc_border_width) + number($main.nShadowOffset)"/>px, <xsl:value-of select="number($main.nTabBGHeight) + 2*number(desc_border_width) + number($main.nShadowOffset)"/>px, 0); border-width: <xsl:value-of select="desc_border_width"/>px <xsl:value-of select="desc_border_width"/>px <xsl:value-of select="desc_border_width"/>px 0;</span>
			<xsl:if test="number($main.nTabBGRadius)!=0">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">border-radius</xsl:attribute>
					<xsl:attribute name="data-value">0 <xsl:value-of select="$main.nTabBGRadius"/>px <xsl:value-of select="$main.nTabBGRadius"/>px 0</xsl:attribute>
				</span>
			</xsl:if>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-bg-top</xsl:attribute>
			<span class="rule-static">clip: rect(-<xsl:value-of select="$main.nShadowOffset"/>px, <xsl:value-of select="number($main.nTabBGWidth) + 2*number(desc_border_width) + number($main.nShadowOffset)"/>px, <xsl:value-of select="number($main.nTabBGHeight) + number(desc_border_width)"/>px, -<xsl:value-of select="$main.nShadowOffset"/>px); border-width: <xsl:value-of select="desc_border_width"/>px <xsl:value-of select="desc_border_width"/>px 0 <xsl:value-of select="desc_border_width"/>px;</span>
			<xsl:if test="number($main.nTabBGRadius)!=0">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">border-radius</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:value-of select="$main.nTabBGRadius"/>px <xsl:value-of select="$main.nTabBGRadius"/>px 0 0</xsl:attribute>
				</span>
			</xsl:if>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-bg-bottom</xsl:attribute>
			<span class="rule-static">clip: rect(0, <xsl:value-of select="number($main.nTabBGWidth) + 2*number(desc_border_width) + number($main.nShadowOffset)"/>px, <xsl:value-of select="number($main.nTabBGHeight) + number(desc_border_width) + number($main.nShadowOffset)"/>px, -<xsl:value-of select="$main.nShadowOffset"/>px); border-width: 0 <xsl:value-of select="desc_border_width"/>px <xsl:value-of select="desc_border_width"/>px <xsl:value-of select="desc_border_width"/>px;</span>
			<xsl:if test="number($main.nTabBGRadius)!=0">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">border-radius</xsl:attribute>
					<xsl:attribute name="data-value">0 0 <xsl:value-of select="$main.nTabBGRadius"/>px <xsl:value-of select="$main.nTabBGRadius"/>px</xsl:attribute>
				</span>
			</xsl:if>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab</xsl:attribute>
			<span class="rule-static">margin: <xsl:value-of select="number($main.nDescWrapperMargin) + number(desc_border_width)"/>px; width: <xsl:value-of select="$main.nTabWidth"/>px; height: <xsl:value-of select="$main.nTabHeight"/>px; border-width: <xsl:value-of select="tab_border_width"/>px;</span>
			<xsl:if test="number(tab_radius)!=0">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">border-radius</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:value-of select="tab_radius"/>px</xsl:attribute>
				</span>
			</xsl:if>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-idle</xsl:attribute>
			<xsl:if test="tab_color_scheme!='spec'"><span class="rule-static">border-color: <xsl:value-of select="$main.sTabColorBorder"/>; background-color: <xsl:value-of select="$main.sTabColor1"/>;</span></xsl:if>
			<xsl:if test="shadow_strength!='none'">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">box-shadow</xsl:attribute>
					<xsl:attribute name="data-value"><xsl:value-of select="$main.sShadowString"/></xsl:attribute>
			...
			<xsl:if test="tab_color_scheme!='spec'">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">linear-gradient</xsl:attribute>
					<xsl:attribute name="data-angle">0</xsl:attribute>
					<xsl:attribute name="data-colors"><xsl:value-of select="$main.sTabColor1"/>|10;<xsl:value-of select="$main.sTabColor3"/>|50;<xsl:value-of select="$main.sTabColor4"/>|50;<xsl:value-of select="$main.sTabColor2"/>|90</xsl:attribute>
				</span>
			</xsl:if>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-over</xsl:attribute>
			<xsl:if test="tab_color_scheme!='spec'"><span class="rule-static">border-color: <xsl:value-of select="$main.sTabColorBorder"/>; background-color: <xsl:value-of select="$main.sTabColor1"/>;</span></xsl:if>
			<xsl:if test="shadow_strength!='none'">
				<span class="rule-dynamic">
					<xsl:attribute name="data-type">box-shadow</xsl:attribute>
		...
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-inner-div</xsl:attribute>
			<span class="rule-static"><xsl:value-of select="$main.sTabFontCSS"/></span>
		</div>
		<xsl:if test="tab_color_scheme!='spec'">
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-idle .cl-tab-inner-div</xsl:attribute>
				<span class="rule-static">color: <xsl:value-of select="$main.sTabColorFont"/>;</span>
			</div>
		</xsl:if>
		<xsl:if test="tab_color_scheme_selected!='spec'">
			...
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-selected .cl-tab-inner-div</xsl:attribute>
				<span class="rule-static">color: <xsl:value-of select="$main.sTabColorSelFont"/>;</span>
			</div>
		</xsl:if>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-table-cell</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="$main.nTabCellWidth"/>px; height: <xsl:value-of select="$main.nTabCellHeight"/>px; padding: <xsl:value-of select="$main.nTabCellPaddingTB"/>px <xsl:value-of select="$main.nTabCellPaddingLR"/>px;</span>
		</div>
		<xsl:for-each select="elements/item">
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-bg-<xsl:value-of select="position()"/></xsl:attribute>
				<span class="rule-static"><xsl:choose><xsl:when test="../../tab_type='left'">left: 0;</xsl:when><xsl:when test="../../tab_type='right'">right: 0;</xsl:when><xsl:when test="../../tab_type='top' or ../../tab_type='bottom'"><xsl:choose><xsl:when test="position()=1">left: 0;</xsl:when><xsl:when test="position()=last()">right: 0;</xsl:when><xsl:otherwise>left: <xsl:value-of select="(position()-1)*number($main.nTabBGStepX)"/>px;</xsl:otherwise></xsl:choose></xsl:when></xsl:choose><xsl:choose><xsl:when test="../../tab_type='top'">top: 0;</xsl:when><xsl:when test="../../tab_type='bottom'">bottom: 0;</xsl:when><xsl:when test="../../tab_type='left' or ../../tab_type='right'"><xsl:choose><xsl:when test="position()=1">top: 0;</xsl:when><xsl:when test="position()=last()">bottom: 0;</xsl:when><xsl:otherwise>top: <xsl:value-of select="(position()-1)*number($main.nTabBGStepY)"/>px;</xsl:otherwise></xsl:choose></xsl:when></xsl:choose></span>
			</div>
			<div class="rule">
				<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-<xsl:value-of select="position()"/></xsl:attribute>
				<span class="rule-static"><xsl:choose><xsl:when test="../../tab_type='left'">left: 0;</xsl:when><xsl:when test="../../tab_type='right'">right: 0;</xsl:when><xsl:when test="../../tab_type='top' or ../../tab_type='bottom'"><xsl:choose><xsl:when test="position()=1">left: 0;</xsl:when><xsl:when test="position()=last()">right: 0;</xsl:when><xsl:otherwise>left: <xsl:value-of select="(position()-1)*number($main.nTabBGStepX)"/>px;</xsl:otherwise></xsl:choose></xsl:when></xsl:choose><xsl:choose><xsl:when test="../../tab_type='top'">top: 0;</xsl:when><xsl:when test="../../tab_type='bottom'">bottom: 0;</xsl:when><xsl:when test="../../tab_type='left' or ../../tab_type='right'"><xsl:choose><xsl:when test="position()=1">top: 0;</xsl:when><xsl:when test="position()=last()">bottom: 0;</xsl:when><xsl:otherwise>top: <xsl:value-of select="(position()-1)*number($main.nTabBGStepY)"/>px;</xsl:otherwise></xsl:choose></xsl:when></xsl:choose></span>
			</div>
			<xsl:if test="../../tab_color_scheme='spec'">
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-idle-<xsl:value-of select="position()"/></xsl:attribute>
					<span class="rule-static">background-color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+1), '|')"/>; border-color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+5), '|')"/>;</span>
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">linear-gradient</xsl:attribute>
						<xsl:attribute name="data-angle">0</xsl:attribute>
						<xsl:attribute name="data-colors"><xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+1), '|')"/>|10;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+3), '|')"/>|50;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+4), '|')"/>|50;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+2), '|')"/>|90</xsl:attribute>
					</span>
				</div>
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-over-<xsl:value-of select="position()"/></xsl:attribute>
					<span class="rule-static">background-color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+2), '|')"/>; border-color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+5), '|')"/>;</span>
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">linear-gradient</xsl:attribute>
						<xsl:attribute name="data-angle">180</xsl:attribute>
						<xsl:attribute name="data-colors"><xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+1), '|')"/>|10;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+3), '|')"/>|50;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+4), '|')"/>|50;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+2), '|')"/>|90</xsl:attribute>
				...
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-idle-<xsl:value-of select="position()"/> .cl-tab-inner-div</xsl:attribute>
					<span class="rule-static">color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+6), '|')"/>;</span>
				</div>
			</xsl:if>
			<xsl:if test="../../tab_color_scheme_selected='spec'">
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-selected-<xsl:value-of select="position()"/></xsl:attribute>
					<span class="rule-static">background-color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+7), '|')"/>; border-color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+11), '|')"/>;</span>
					<span class="rule-dynamic">
						<xsl:attribute name="data-type">linear-gradient</xsl:attribute>
						<xsl:attribute name="data-angle">0</xsl:attribute>
						<xsl:attribute name="data-colors"><xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+7), '|')"/>|10;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+9), '|')"/>|50;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+10), '|')"/>|50;<xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+8), '|')"/>|90</xsl:attribute>
					</span>
				</div>
				<div class="rule">
					<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-tab-selected-<xsl:value-of select="position()"/> .cl-tab-inner-div</xsl:attribute>
					<span class="rule-static">color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+12), '|')"/>;</span>
				</div>
			</xsl:if>
		</xsl:for-each>
	</div>
	<div>
		...
		<div>
			...
			<div>
				...
				<div>
					<xsl:attribute name="class">cl-desc-bg-<xsl:value-of select="tab_type"/> cl-desc-bg <xsl:choose><xsl:when test="count(elements/item)=0"> cl-desc-empty </xsl:when><xsl:when test="count(elements/item)=1"> cl-desc-bg-single </xsl:when><xsl:otherwise><xsl:choose><xsl:when test="number($main.iItemToPreview)=1"> cl-desc-bg-first </xsl:when><xsl:when test="number($main.iItemToPreview)=count(elements/item)"> cl-desc-bg-last </xsl:when><xsl:otherwise> cl-desc-bg-regular</xsl:otherwise></xsl:choose></xsl:otherwise></xsl:choose></xsl:attribute>
					<div>
						<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_DESC_WRAPPER</xsl:attribute>
						<xsl:attribute name="class">cl-desc-wrapper</xsl:attribute>
						<xsl:if test="number($main.iItemToPreview)=0">
				...
				<div>
					<xsl:attribute name="class">cl-tab-part-<xsl:value-of select="tab_type"/></xsl:attribute>
					<xsl:for-each select="elements/item">
						<xsl:call-template name="tab-bg">
							<xsl:with-param name="param.iLevelNumber" select="position()"/>
							<xsl:with-param name="param.iLevelToPreview" select="$main.iItemToPreview"/>
						</xsl:call-template>
					</xsl:for-each>
...
<xsl:template name="tab-bg">
	<xsl:param name="param.iLevelNumber"/>
	<xsl:param name="param.iLevelToPreview"/>

	<div>
		<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_TAB_BG_<xsl:value-of select="$param.iLevelNumber"/></xsl:attribute>
		<xsl:attribute name="class">cl-tab-bg unselectable cl-tab-bg-<xsl:value-of select="../../tab_type"/> cl-tab-bg-<xsl:value-of select="$param.iLevelNumber"/></xsl:attribute>
		<xsl:attribute name="style"><xsl:if test="number($param.iLevelToPreview)!=number($param.iLevelNumber)">display: none;</xsl:if></xsl:attribute>
		...
		<table>
			...
			<tr>
				<td>
					...
					<div>
						<xsl:attribute name="class">cl-tab-inner-div</xsl:attribute>
						<xsl:choose>
							<xsl:when test="text_type='html'"><xsl:value-of select="tab_text_html" disable-output-escaping="yes"/></xsl:when>
							<xsl:otherwise><xsl:value-of select="tab_text"/></xsl:otherwise>
						</xsl:choose>
					</div>
				</td>
			</tr>
...
<xsl:template name="desc">
	...
	<div>
		...
		<div>
			<xsl:attribute name="class">cl-desc-inner</xsl:attribute>
			<xsl:choose>
