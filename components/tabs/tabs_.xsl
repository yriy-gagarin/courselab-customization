...
<xsl:template match="params">
	...
	<style>
		...
		.cl-desc-wrapper { margin: <xsl:value-of select="$main.nDescWrapperMargin"/>px; padding: <xsl:value-of select="desc_padding"/>px; width: <xsl:value-of select="number($main.nDescWrapperWidth) - 2*number(desc_border_width) - 2*number(desc_padding)"/>px; height: <xsl:value-of select="number($main.nDescWrapperHeight) - 2*number(desc_border_width) - 2*number(desc_padding)"/>px; background-color: <xsl:value-of select="$main.sDescColor1"/>; border-width: <xsl:value-of select="desc_border_width"/>px; border-color: <xsl:value-of select="$main.sDescBorderColor"/>; border-radius: <xsl:value-of select="desc_radius"/>px; <xsl:if test="$main.sDescColor1!=$main.sDescColor2">background-image: linear-gradient(<xsl:value-of select="desc_bg_grad_angle"/>deg, <xsl:value-of select="$main.sDescColor1"/>, <xsl:value-of select="$main.sDescColor2"/>);</xsl:if> }
		...
		.cl-tab-idle { <xsl:if test="tab_color_scheme!='spec'">border-color: <xsl:value-of select="$main.sTabColorBorder"/>; background-color: <xsl:value-of select="$main.sTabColor1"/>; background-image: linear-gradient(0deg, <xsl:value-of select="$main.sTabColor1"/> 10%, <xsl:value-of select="$main.sTabColor3"/> 50%, <xsl:value-of select="$main.sTabColor4"/> 50%, <xsl:value-of select="$main.sTabColor2"/> 90%);</xsl:if> <xsl:if test="tab_shadow!='none'">box-shadow: <xsl:value-of select="$main.sShadowString"/>;</xsl:if> }
		.cl-tab-selected { <xsl:if test="tab_color_scheme_selected!='spec'">border-color: <xsl:value-of select="$main.sTabColorSelBorder"/>; background-color: <xsl:value-of select="$main.sTabColorSel1"/>; background-image: linear-gradient(0deg, <xsl:value-of select="$main.sTabColorSel1"/> 10%, <xsl:value-of select="$main.sTabColorSel3"/> 50%, <xsl:value-of select="$main.sTabColorSel4"/> 50%, <xsl:value-of select="$main.sTabColorSel2"/> 90%);</xsl:if> }
		.cl-tab-inner-div { <xsl:value-of select="$main.sTabFontCSS"/> }
		.cl-tab-idle .cl-tab-inner-div { <xsl:if test="tab_color_scheme!='spec'">color: <xsl:value-of select="$main.sTabColorFont"/>;</xsl:if> }
		.cl-tab-selected .cl-tab-inner-div { <xsl:if test="tab_color_scheme_selected!='spec'">color: <xsl:value-of select="$main.sTabColorSelFont"/>;</xsl:if> }
		.cl-tab-table-cell { width: <xsl:value-of select="$main.nTabCellWidth"/>px; height: <xsl:value-of select="$main.nTabCellHeight"/>px; padding: <xsl:value-of select="$main.nTabCellPaddingTB"/>px <xsl:value-of select="$main.nTabCellPaddingLR"/>px; }
		<xsl:for-each select="elements/item">
			.cl-tab-bg-<xsl:value-of select="position()"/> { <xsl:choose><xsl:when test="../../tab_type='left'">left: 0;</xsl:when><xsl:when test="../../tab_type='right'">right: 0;</xsl:when><xsl:when test="../../tab_type='top' or ../../tab_type='bottom'"><xsl:choose><xsl:when test="position()=1">left: 0;</xsl:when><xsl:when test="position()=last()">right: 0;</xsl:when><xsl:otherwise>left: <xsl:value-of select="(position()-1)*number($main.nTabBGStepX)"/>px;</xsl:otherwise></xsl:choose></xsl:when></xsl:choose><xsl:choose><xsl:when test="../../tab_type='top'">top: 0;</xsl:when><xsl:when test="../../tab_type='bottom'">bottom: 0;</xsl:when><xsl:when test="../../tab_type='left' or ../../tab_type='right'"><xsl:choose><xsl:when test="position()=1">top: 0;</xsl:when><xsl:when test="position()=last()">bottom: 0;</xsl:when><xsl:otherwise>top: <xsl:value-of select="(position()-1)*number($main.nTabBGStepY)"/>px;</xsl:otherwise></xsl:choose></xsl:when></xsl:choose> }
			.cl-tab-<xsl:value-of select="position()"/> { <xsl:choose><xsl:when test="../../tab_type='left'">left: 0;</xsl:when><xsl:when test="../../tab_type='right'">right: 0;</xsl:when><xsl:when test="../../tab_type='top' or ../../tab_type='bottom'"><xsl:choose><xsl:when test="position()=1">left: 0;</xsl:when><xsl:when test="position()=last()">right: 0;</xsl:when><xsl:otherwise>left: <xsl:value-of select="(position()-1)*number($main.nTabBGStepX)"/>px;</xsl:otherwise></xsl:choose></xsl:when></xsl:choose><xsl:choose><xsl:when test="../../tab_type='top'">top: 0;</xsl:when><xsl:when test="../../tab_type='bottom'">bottom: 0;</xsl:when><xsl:when test="../../tab_type='left' or ../../tab_type='right'"><xsl:choose><xsl:when test="position()=1">top: 0;</xsl:when><xsl:when test="position()=last()">bottom: 0;</xsl:when><xsl:otherwise>top: <xsl:value-of select="(position()-1)*number($main.nTabBGStepY)"/>px;</xsl:otherwise></xsl:choose></xsl:when></xsl:choose> }
			<xsl:if test="../../tab_color_scheme='spec'">
				...
				.cl-tab-idle-<xsl:value-of select="position()"/> .cl-tab-inner-div { color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+6), '|')"/>; }
			</xsl:if>
			<xsl:if test="../../tab_color_scheme_selected='spec'">
				...
				.cl-tab-selected-<xsl:value-of select="position()"/> .cl-tab-inner-div { color: <xsl:value-of select="websoft:arrayElem(string($main.arTabColors), (12*(position()-1)+12), '|')"/>; }
			</xsl:if>
		</xsl:for-each>
	</style>
	<div>
		<xsl:attribute name="class">clo-v_rapid_001</xsl:attribute>
		<div>
